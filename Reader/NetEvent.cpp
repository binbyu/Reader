...
{
    unsigned int i;
    for (i = 0; i < m_readfds.fd_count; i++)
        Del(m_readfds.fd_array[i]);
    for (i = 0; i < m_writefds.fd_count; i++)
        Del(m_writefds.fd_array[i]);
    for (i = 0; i < m_exceptfds.fd_count; i++)
        Del(m_exceptfds.fd_array[i]);
    return 0;
}
