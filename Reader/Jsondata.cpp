...
public:
    json_font_t(cJSON* parent, LOGFONT* data)
    {
        ...
        lfFaceName = cJSON_AddStringToObject(parent, "lfFaceName", Utils::Utf16ToUtf8(data->lfFaceName));
    }
    ...
    void GetData(LOGFONT* data)
    {
        ...
        if (lfItalic)
            data->lfItalic = lfItalic->valueint;
        if (lfUnderline)
            data->lfUnderline = lfUnderline->valueint;
        if (lfStrikeOut)
            data->lfStrikeOut = lfStrikeOut->valueint;
        if (lfCharSet)
            data->lfCharSet = lfCharSet->valueint;
        if (lfOutPrecision)
            data->lfOutPrecision = lfOutPrecision->valueint;
        if (lfClipPrecision)
            data->lfClipPrecision = lfClipPrecision->valueint;
        if (lfQuality)
            data->lfQuality = lfQuality->valueint;
        if (lfPitchAndFamily)
            data->lfPitchAndFamily = lfPitchAndFamily->valueint;
        if (lfFaceName)
            wcscpy(data->lfFaceName, Utils::Utf8ToUtf16(lfFaceName->valuestring));
    }
};
...
public:
    json_bg_image_t(cJSON* parent, bg_image_t* data)
    {
        ...
        file_name = cJSON_AddStringToObject(parent, "file_name", Utils::Utf16ToUtf8(data->file_name));
    }
    ...
    void GetData(bg_image_t* data)
    {
        ...
        if (file_name)
            wcscpy(data->file_name, Utils::Utf8ToUtf16(file_name->valuestring));
    }
};
...
public:
    json_proxy_t(cJSON* parent, proxy_t* data)
    {
        ...
        addr = cJSON_AddStringToObject(parent, "addr", Utils::Utf16ToUtf8(data->addr));
        user = cJSON_AddStringToObject(parent, "user", Utils::Utf16ToUtf8(data->user));
        pass = cJSON_AddStringToObject(parent, "pass", Utils::Utf16ToUtf8(data->pass));
    }
    ...
    void GetData(proxy_t* data)
    {
        ...
        if (addr)
            wcscpy(data->addr, Utils::Utf8ToUtf16(addr->valuestring));
        if (user)
            wcscpy(data->user, Utils::Utf8ToUtf16(user->valuestring));
        if (pass)
            wcscpy(data->pass, Utils::Utf8ToUtf16(pass->valuestring));
    }
};
...
public:
    json_chapter_rule_t(cJSON* parent, chapter_rule_t* data)
    {
        ...
        keyword = cJSON_AddStringToObject(parent, "keyword", Utils::Utf16ToUtf8(data->keyword));
        regex = cJSON_AddStringToObject(parent, "regex", Utils::Utf16ToUtf8(data->regex));
    }
    ...
    void GetData(chapter_rule_t* data)
    {
        ...
        if (keyword)
            wcscpy(data->keyword, Utils::Utf8ToUtf16(keyword->valuestring));
        if (regex)
            wcscpy(data->regex, Utils::Utf8ToUtf16(regex->valuestring));
    }
};
...
public:
    json_header_t(cJSON* parent, header_t* data)
    {
        ...
        item = cJSON_CreateObject();
        rect = new json_rect_t(item, &(data->rect));
        cJSON_AddItemToObject(parent, "rect", item);
    ...
    void GetData(header_t* data)
    {
        ...
        if (version)
            wcscpy(data->version, Utils::Utf8ToUtf16(version->valuestring));
        if (item_count)
        ...
        if (keyset)
        {
            size = cJSON_GetArraySize(keyset);
            if (size > KI_MAXCOUNT)
                size = KI_MAXCOUNT;
            for (i = 0; i < size; i++)
            {
                item = cJSON_GetArrayItem(keyset, i);
                if (item)
                    data->keyset[i] = *((u32*)&item->valueint);
            }
        }
...
bool parser_json(const char* json, header_t* default, void** data, int* size)
{
    ...
    *size = 0;
    ...
    headerobj->GetData(default);
    delete headerobj;

    // parser items
    items = cJSON_GetObjectItem(root, "items");
    if (!items || cJSON_GetArraySize(items) <= 0 || cJSON_GetArraySize(items) != default->item_count)
    {
    ...
    *size = sizeof(header_t) + (sizeof(item_t) * (default->item_count));
    *data = (header_t*)malloc(*size);
    memset(*data, 0, *size);
    memcpy(*data, default, sizeof(header_t));

    for (i = 0; i < default->item_count; i++)
    {
        offset = sizeof(header_t) + i * sizeof(item_t);
        itemdata = (item_t*)((char*)(*data) + offset);
