...
static void _enable_content_next(HWND hDlg)
{
    ...
    if (iPos == 0)
    {
        ...
        EnableWindow(GetDlgItem(hDlg, IDC_EDIT_CTX_KEYWORD_CTX), FALSE);
    }
    else
    {
        EnableWindow(GetDlgItem(hDlg, IDC_EDIT_CTX_NEXT), TRUE);
        EnableWindow(GetDlgItem(hDlg, IDC_EDIT_CTX_KEYWORD), TRUE);
        EnableWindow(GetDlgItem(hDlg, IDC_EDIT_CTX_KEYWORD_CTX), TRUE);
    }
}
...
static void _clear_ui(HWND hDlg)
{
    ...
    SetDlgItemText(hDlg, IDC_EDIT_CTX_KEYWORD_CTX, _T(""));
...
static void _set_data_to_ui(HWND hDlg, const book_source_t *data)
{
    ...
    SetDlgItemText(hDlg, IDC_EDIT_HOST, Utils::Utf8ToUtf16(data->host));
    SetDlgItemText(hDlg, IDC_EDIT_QUERY, Utils::Utf8ToUtf16(data->query_url));
    SetDlgItemText(hDlg, IDC_EDIT_POST, Utils::Utf8ToUtf16(data->query_params));
    SetDlgItemText(hDlg, IDC_EDIT_NAME, Utils::Utf8ToUtf16(data->book_name_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_MAINPAGE, Utils::Utf8ToUtf16(data->book_mainpage_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_AUTHOR, Utils::Utf8ToUtf16(data->book_author_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_LISTPAGE, Utils::Utf8ToUtf16(data->chapter_page_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_CPT_TITLE, Utils::Utf8ToUtf16(data->chapter_title_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_CPT_URL, Utils::Utf8ToUtf16(data->chapter_url_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_CTX, Utils::Utf8ToUtf16(data->content_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_CTX_NEXT, Utils::Utf8ToUtf16(data->content_next_url_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_CTX_KEYWORD, Utils::Utf8ToUtf16(data->content_next_keyword_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_CTX_KEYWORD_CTX, Utils::Utf8ToUtf16(data->content_next_keyword));
    SetDlgItemText(hDlg, IDC_EDIT_STATUS, Utils::Utf8ToUtf16(data->book_status_xpath));
    SetDlgItemText(hDlg, IDC_EDIT_KEYWORD, Utils::Utf8ToUtf16(data->book_status_keyword));
...
static void _get_data_from_ui(HWND hDlg, book_source_t *data)
{
    ...
    strcpy(data->host, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_QUERY, buf, 1024);
    strcpy(data->query_url, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_POST, buf, 1024);
    strcpy(data->query_params, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_NAME, buf, 1024);
    strcpy(data->book_name_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_MAINPAGE, buf, 1024);
    strcpy(data->book_mainpage_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_AUTHOR, buf, 1024);
    strcpy(data->book_author_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_LISTPAGE, buf, 1024);
    strcpy(data->chapter_page_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_CPT_TITLE, buf, 1024);
    strcpy(data->chapter_title_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_CPT_URL, buf, 1024);
    strcpy(data->chapter_url_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_CTX, buf, 1024);
    strcpy(data->content_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_CTX_NEXT, buf, 1024);
    strcpy(data->content_next_url_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_CTX_KEYWORD, buf, 1024);
    strcpy(data->content_next_keyword_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_CTX_KEYWORD_CTX, buf, 1024);
    strcpy(data->content_next_keyword, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_STATUS, buf, 1024);
    strcpy(data->book_status_xpath, Utils::Utf16ToUtf8(buf));
    GetDlgItemText(hDlg, IDC_EDIT_KEYWORD, buf, 1024);
    strcpy(data->book_status_keyword, Utils::Utf16ToUtf8(buf));
...
static BOOL _check_is_empty(HWND hDlg, book_source_t *data)
{
    ...
    if (!data)
    {
        memset(&temp, 0, sizeof(book_source_t));
        _get_data_from_ui(hDlg, &temp);
        data = &temp;
    }
    if (!data->title[0])
        goto _yes;
...
static BOOL _check_is_modify(HWND hDlg, int idx, book_source_t *data)
{
    ...
    return 0 != memcmp(data, &_header->book_sources[idx], sizeof(book_source_t));
}

BOOL ParserHost(const char* url, char* host)
{
    const char* p = NULL;
    const char* t = NULL;
...
void CombineUrl(const char *path, const char *url, char *dsturl)
{
    char temp[1024] = {0};
    char host_[1024] = { 0 };
    char *p;
    ...
    else
    {
        if (path[0] == '/')
        {
            ParserHost(url, host_);
            sprintf(dsturl, "%s%s", host_, path);
        }
        else
        {
            strcpy(temp, url);
...
BOOL Redirect(request_t *r, const char *url, req_handler_t *hReq)
{
    request_t req;
    char url_[1024] = { 0 };
#if TEST_MODEL
...
static void EnableDialog_Sync(HWND hDlg, BOOL enable)
{
    ...
    EnableWindow(GetDlgItem(hDlg, IDC_EDIT_CTX_KEYWORD_CTX), enable);
    ...
    if (enable)
    {
        memset(&data, 0, sizeof(book_source_t));
        _get_data_from_ui(hDlg, &data);
...
static unsigned int DownloadBooksrcCompleter(request_result_t* result)
{
    ...
    {
        // redirect
        if (Redirect(result->req, hapi_get_location(result->header), &g_hRequestSync))
        {
            return 0;
        }
        EnableDialog_Sync(hDlg, TRUE);
...
static INT_PTR CALLBACK BS_DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            ...
            // check empty
            if (_check_is_empty(hDlg, &data))
            {
                return (INT_PTR)FALSE;
            }

            // check is exist
            if (_check_is_exist(hDlg, -1, &data))
            {
                return (INT_PTR)FALSE;
            }
            ...
            memcpy(&_header->book_sources[iPos], &data, sizeof(book_source_t));
        ...
        case IDC_SAVE:
            ...
            // check is exist
            if (_check_is_exist(hDlg, iPos, &data))
            {
                return (INT_PTR)FALSE;
            }

            // check is modify
            if (!_check_is_modify(hDlg, iPos, &data))
            {
            ...
            if (0 != _tcscmp(_header->book_sources[iPos].title, data.title)
                && 0 != strcmp(_header->book_sources[iPos].host, data.host))
            {
            ...
            if (iPos >= 0 && iPos < _header->book_source_count)
            {
                ...
                if (hMenu)
                {
                    ...
                    if (iPos > 0)
                    {
                        ...
                        InsertMenu(hMenu, -1, MF_BYPOSITION, IDM_BS_MOVE_UP, str);
                    }
                    if (iPos < _header->book_source_count-1)
                    {
                        LoadString(hInst, IDS_MOVE_DOWN, str, 256);
                        InsertMenu(hMenu, -1, MF_BYPOSITION, IDM_BS_MOVE_DOWN, str);
                    }
